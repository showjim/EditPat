...
def main():
    ...
    st.subheader('Powered by Streamlit, written by Chao ZHou', divider='rainbow')
    ...
    st.caption('Step 1. Upload atp/atp.gz files')
    ...
    options = st.multiselect('Select files you want to process',
                             index_file_list)
    if len(options) > 0:
    ...
    st.caption('Step 2. Upload or create CSV config file')
    ## 2.1
    ...
    ## 2.2
    file_path = st.file_uploader("2.2.Upload a CSV config file",
                                 type=["csv"])
    ...
    st.caption('Step 3. Run post-process')
